...
const createHtmlFiles = (filePath, fileType) => {
  fs.readFile(filePath, "utf8", (err, data) => {
    if (err) return console.error(`Unable to read file ${filePath}`, err);
...
const markdownToHtml = (param) => {
  ...
  if (param.match(/^\s*#{1,6}[^#]+$/)) {
    ...
    if (param.match(/\[(.+)\]\((.+)\)/))
      return Object({
        type: "a",
        attributes: { href: param.match(/\[(.+)\]\((.+)\)/)[2] },
        content: param.match(/\[(.+)\]\((.+)\)/)[1],
      });
    if (param.match(/---/)) return Object({ type: "hr", content: null });
    return Object({ type: "p", content: param });
  }
};
...
const processInput = (filepath) => {
  ...
  fs.readdirSync(outputPath).forEach((file) => {
    ...
    if (
      filePaths.indexOf(outputFolderFile) < 0 &&
      outputFolderFile.split(".").pop() == "html"
    ) {
  ...
  indexHtml.document.addElementToType("body", {
    type: "div",
    content: linkObj,
  });
  fs.writeFile(
    ...
    indexHtml
      ...
      .replace(
        /<html>/,
        `<html lang="${option.lang ? option.lang : defaultLang}">`
      ),
    (err) => {
      if (err) return console.error(`Unable to create index.html file`, err);
...
if (option.config) {
  ...
  if (!fs.existsSync(option.config))
    console.error(
      `Config file "${option.config}" doesn't exist, please check file path.`
    );
  else {
    //parse JSON contents
    ...
    if (configOpts) {
      if (configOpts.output && fs.existsSync(configOpts.output)) {
        ...
        if (tempPath.isDirectory()) outputPath = configOpts.output;
      }
      ...
      if (configOpts.input && fs.existsSync(configOpts.input)) {
        ...
        console.error(
          `error: input '<file path>' not specified in config file`
        );
      else
        console.error(
          `error: no file or directory at input file path ${configOpts.input}, please check file path`
        );
    } else
  ...
  if (option.output) {
    ...
    if (tempPath.isDirectory()) outputPath = option.output;
  }

  if (option.input) processInput(option.input);
  else
    console.error(
